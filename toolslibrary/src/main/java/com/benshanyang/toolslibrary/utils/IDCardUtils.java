...
public class IDCardUtils {
    ...
    public static boolean validate(String idCardNumber) {
        ...
        if (idNumber.length() == 15 || idNumber.length() == 18) {
            //如果为15位则自动补全到18位
            if (idNumber.length() == OLD_CARD_NUMBER_LENGTH) {
                idNumber = contertToNewCardNumber(idNumber);
            }
        } else {
            errorInfo = LENGTH_ERROR;
            return false;
        }
        ...
        try {
            ...
            if (null == birthDate || !birthDate.before(new Date()) || !birthDate.after(MINIMAL_BIRTH_DATE)) {
                //DATE_ERROR
                errorInfo = DATE_ERROR;
                return false;
            }
            //出生日期中的年、月、日必须正确,比如月份范围是[1,12],日期范围是[1,31]，还需要校验闰年、大月、小月的情况时，月份和日期相符合
            String birthdayPart = getBirthDayPart(idNumber);
        ...
        if (h.get(idNumber.substring(0, 2)) == null) {
            //AREA_ERROR
            errorInfo = AREA_ERROR;
            return false;
        }
        //校验身份证最后一位 身份证校验码
        if (!TextUtils.equals(calculateVerifyCode(idNumber), String.valueOf(idNumber.charAt(NEW_CARD_NUMBER_LENGTH - 1)))) {
            errorInfo = CHECKCODE_ERROR;
            return false;
        }
        //通过验证则返回true
        errorInfo = "";
    ...
    private static Date getBirthDate(String idCard) {
        ...
        return cacheBirthDate != null ? new Date(cacheBirthDate.getTime()) : null;
    }
